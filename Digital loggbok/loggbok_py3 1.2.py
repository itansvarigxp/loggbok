...
# Funktion för att betsämma när listan ska tömmas
def clear_incheckade():
    while True:
        time.sleep(3600)  # Pausa 60*60 sekunder = 3600 = en timme
        today = datetime.datetime.now()
        time_now = today.time().strftime("%H:%M:%S")  # kolla klockan
        if time_now >= ('04:00:00') and time_now <= ('05:00:00'):  # Mellan 4 & 5
            typewrite('Rensa listan')
            press('enter')
            
t = Thread(target=clear_incheckade)
t.start()

# Separat tråd för att spara listan externt loggbok.save('info/Loggbok_extern.xlsx')
def spara():
    while True:
        time.sleep(1800)  # Spara var 30:e minut, 60*30 sekunder
        loggbok.save('info/Loggbok_extern.xlsx')
        
t2 = Thread(target=spara)
t2.start()
